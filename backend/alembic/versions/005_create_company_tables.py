...
def upgrade() -> None:
    ...
    if "company_configs" not in tables:
        op.create_table(
            "company_configs",
            sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("uuid_generate_v4()")),
            sa.Column("infrastructure_id", UUID(as_uuid=True), sa.ForeignKey("infrastructures.id", ondelete="CASCADE"), nullable=False, unique=True),
            sa.Column("heartbeat_interval_seconds", sa.Integer, server_default="1800"),
            sa.Column("default_model", sa.String(255), server_default="claude-haiku-4-5-20251001"),
    ...
    if "company_departments" not in tables:
        op.create_table(
            ...
            sa.Column("parent_id", UUID(as_uuid=True), sa.ForeignKey("company_departments.id", ondelete="SET NULL"), nullable=True),
    ...
    # -- CompanyRole --
    if "company_roles" not in tables:
        op.create_table(
            "company_roles",
            sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("uuid_generate_v4()")),
            sa.Column("infrastructure_id", UUID(as_uuid=True), sa.ForeignKey("infrastructures.id", ondelete="CASCADE"), nullable=False),
            ...
            sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
            sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
            sa.UniqueConstraint("infrastructure_id", "agent_slug", name="uq_company_role_agent"),
        )

    # -- CompanySchedule --
    if "company_schedules" not in tables:
        op.create_table(
            "company_schedules",
            sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("uuid_generate_v4()")),
            sa.Column("infrastructure_id", UUID(as_uuid=True), sa.ForeignKey("infrastructures.id", ondelete="CASCADE"), nullable=False),
    ...
    # -- CompanyTask --
    if "company_tasks" not in tables:
        op.create_table(
            "company_tasks",
            sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("uuid_generate_v4()")),
            sa.Column("infrastructure_id", UUID(as_uuid=True), sa.ForeignKey("infrastructures.id", ondelete="CASCADE"), nullable=False),
            sa.Column("schedule_id", UUID(as_uuid=True), sa.ForeignKey("company_schedules.id", ondelete="SET NULL"), nullable=True),
            sa.Column("agent_slug", sa.String(255), nullable=False),
            sa.Column("title", sa.String(500), nullable=False),
            sa.Column("description", sa.Text, nullable=True),
    ...
    # -- CompanyWorkLog --
    if "company_work_logs" not in tables:
        op.create_table(
            "company_work_logs",
            sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("uuid_generate_v4()")),
            sa.Column("infrastructure_id", UUID(as_uuid=True), sa.ForeignKey("infrastructures.id", ondelete="CASCADE"), nullable=False),
            ...
            sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
            sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
            sa.UniqueConstraint("infrastructure_id", "agent_slug", "log_date", name="uq_company_worklog_agent_date"),
        )
